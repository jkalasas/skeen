<script lang="ts">
	...
	import { Heart, AlertCircle, User, Info, Plus, LogIn } from '@lucide/svelte';
...
<div class="container mx-auto max-w-7xl px-4">
    ...
    <div class="py-12 sm:py-16 text-center space-y-6 animate-in fade-in zoom-in-95 duration-1000">
        <div class="inline-flex items-center justify-center p-3 rounded-2xl bg-accent/10 mb-4 ring-1 ring-accent/20 shadow-inner">
            <Heart class="h-10 w-10 text-accent" />
        </div>
        <h1 class="text-4xl sm:text-6xl font-extrabold tracking-tight text-foreground drop-shadow-sm">
            Perfect <span class="text-gradient">Pairs.</span>
        </h1>
        <p class="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Assess how well your products work together. Build your dream routine.
        </p>
        {#if authStore.initialized && !authStore.isAuthenticated}
            <div class="mt-8 flex justify-center">
                 <Button onclick={() => authStore.signInWithGoogle()} size="lg" class="rounded-full gap-2 shadow-lg shadow-accent/20 animate-pulse-slow">
                    <LogIn class="h-5 w-5" />
                    Sign In to Check
                </Button>
            </div>
    ...
    <AuthGuard>
		...
		<div class="glass rounded-3xl p-6 sm:p-8 mb-8 shadow-2xl ring-1 ring-white/20 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
			<div class="flex items-center justify-between mb-6">
				<h2 class="text-2xl font-bold flex items-center gap-2">
                    <span class="bg-accent/10 text-accent px-3 py-1 rounded-full text-sm font-extrabold">{validProductCount}/{products.length}</span>
                    Products
                </h2>
				<Button
					onclick={addProduct}
					...
					class="gap-2 rounded-full border-accent/20 hover:border-accent hover:bg-accent/5"
				>
					<Plus class="h-4 w-4" />
					Add Product
				</Button>
            ...
            <div class="mt-8 pt-8 border-t border-border/50 flex flex-wrap gap-4 justify-end">
                ...
                <Button
                    ...
                    class="gap-2 rounded-full shadow-lg shadow-accent/20 bg-accent hover:bg-accent/90"
                    size="lg"
                >
                    <Heart class="h-5 w-5" />
		...
		<!-- Combination Results -->
		{#if combination}
            <div class="glass-card rounded-3xl overflow-hidden ring-2 ring-accent/20 shadow-accent/10 shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
